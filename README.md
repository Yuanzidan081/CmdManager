...
- 全局变量管理
  - 支持把常用环境变量保存到 `globalVariables`，在模板中使用 `$变量名$` 引用。
  - 在命令编辑页可通过“插入全局变量”弹层快速检索并插入变量。
- 命令预览与执行
  - 预览区实时展示替换后的命令。
  - 点击运行后在新的终端窗口执行最终命令。
- 数据持久化
...
4. 如需复用常用路径或环境变量，点击“插入全局变量”并选择 `$变量名$` 插入模板。
5. 根据自动生成的变量输入项填写 value。
6. 点击“保存”返回列表页。
7. 点击顶部“设置”进入全局变量管理页，新增、编辑或删除 `globalVariables`。
8. 点击顶部“保存”将当前状态写入 `data/commands.json`。
9. 在命令卡片点击“运行”，将在新终端窗口执行最终命令。
...
│  │  ├─ CategoryModel.py
│  │  ├─ CommandModel.py
  │  │  ├─ GlobalVariableModel.py
  │  │  └─ SegmentModel.py
│  ├─ Services/
│  │  ├─ CategoryService.py
  │  │  ├─ CommandService.py
  │  │  └─ GlobalVariableService.py
│  └─ UI/
│     ├─ MainWindow.py
│     ├─ styles/
│     │  └─ theme.qss
  ...
  "globalVariables": [
    {
      "key": "UNITY_ROOT",
      "value": "E:\\unity",
      "description": "Unity 安装根目录"
    }
  ]
}
```

在模板中，`$UNITY_ROOT$` 会先替换为全局变量值，再继续替换 `%appName%`、`%version%` 这类命令变量。

## 命令拼接与转义

- CommandService 负责模板变量替换和预览拼接，UI 不直接拼接命令字符串。
- 替换顺序为：先处理 `$变量名$`，再处理 `%变量名%`。
...
- “设置”按钮已用于进入全局变量管理页，支持新增、编辑、删除和保存 `globalVariables`。
