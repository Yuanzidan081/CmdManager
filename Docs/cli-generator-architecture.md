...
- 主区：命令卡片列表（每张卡片包含命令名称、名称下方命令预览、右侧操作按钮）。
  - 命令预览为单行省略显示，悬停卡片可查看完整命令。
  - 卡片按钮为复制、运行、编辑、删除，复制按钮在第一个位置。
  - 横向拉伸时，按钮保持固定尺寸不拉伸，文本区与按钮区的间距增大。
  - 主区列表仅纵向滚动，不显示横向滚动条，卡片右侧操作按钮始终可见。
...
- CommandCardWidget：命令卡片，展示命令名称和名称下方的单行预览；支持复制、运行、编辑、删除操作，点击编辑进入CommandEditorWidget子界面。
